...
export const Sparkline = memo(function Sparkline({ values }: SparklineProps) {
  ...
  if (values.length === 0) {
    return <div className="h-16 rounded-lg bg-panel-soft" />;
  }

  return (
    <div className="flex h-20 items-end gap-1 rounded-lg bg-panel-soft p-2">
      {bars.map((height, index) => (
        <div
          key={`${height}-${index}`}
          className="flex-1 rounded-t bg-good"
          style={{ height: `${height}%` }}
          aria-hidden="true"
        />
      ))}
